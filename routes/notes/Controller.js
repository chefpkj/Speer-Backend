...
const getSpecificNotes = async (req, res) => {
  const result = await DbLayer?.getSpecificNotes({
    _id: req.body._id,
...
const updateNotes = async (req, res) => {
  ...
  const result = await DbLayer.updateNotes({
    _id: req?.body?._id,
    notesId: req.params?.id,
...
const deleteNotes = async (req, res) => {
  const result = await DbLayer.deleteNotes({
    _id: req?.body?._id,
    notesId: req.params?.id,
  });
  if (result?.status === 200) {
...
const shareNotes = async (req, res) => {
  const result = await DbLayer.shareNotes({
    _id: req?.body?._id,
    notesId: req.params?.id,
  });
  if (result?.status === 200) {
...
const search = async (req, res) => {
  ...
  const result = await DbLayer.search({
    _id: req?.body?._id,
    keyword: req.query.q,
  });
  if (result?.status === 200) {
